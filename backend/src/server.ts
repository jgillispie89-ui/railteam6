...
// =============================================================================
// GET /api/sites
// Returns GeoJSON FeatureCollection of sites.
// Optional filters:
//   ?bbox=minLng,minLat,maxLng,maxLat  — only sites inside this box
//   ?types=depot,bridge                — comma-separated site_type values
//   ?statuses=active,abandoned         — comma-separated site_status values
//   ?year=1920                         — only sites built by this year (timeline)
//   ?railroad=L&N                      — railroad name or abbreviation match
// =============================================================================
    ...
    try {
        ...
        if (typeof req.query.year === 'string') {
            ...
            if (Number.isFinite(year)) {
                // "exists by year": built by then and not demolished before then
        ...
        if (typeof req.query.railroad === 'string') {
            ...
            params.push(`%${req.query.railroad}%`);
            idx++;
        }
        ...
        const features = rows.map(r => ({
            ...
            properties: {
                ...
                demolished_year: r.demolished_year,
                city: r.city,
                state: r.state_province,
                country: r.country,
                description: r.description,
                railroads: r.railroads,
            },
        }));
...
// =============================================================================
// GET /api/sites/:id  — full detail for one site
// =============================================================================
app.get('/api/sites/:id', async (req, res) => {
    try {
        const { rows: siteRows } = await pool.query(
            `SELECT s.*, ST_AsGeoJSON(s.geom)::json AS geometry
        ...
        const { rows: railroads } = await pool.query(
            `SELECT r.* FROM railroads r
             JOIN site_railroads sr ON sr.railroad_id = r.id
             WHERE sr.site_id = $1`,
            [req.params.id]
        );
        const { rows: photos } = await pool.query(
            `SELECT * FROM photos WHERE site_id = $1 ORDER BY taken_year NULLS LAST`,
            [req.params.id]
        );
        const { rows: sources } = await pool.query(
            `SELECT * FROM sources WHERE site_id = $1`,
            [req.params.id]
        );
...
// =============================================================================
// GET /api/lines — rail corridors as GeoJSON, with same filters as sites
// =============================================================================
app.get('/api/lines', async (req, res) => {
    try {
        ...
        if (typeof req.query.statuses === 'string') {
            ...
            params.push(req.query.statuses.split(','));
            idx++;
        }
        if (typeof req.query.year === 'string') {
            const year = parseInt(req.query.year, 10);
        ...
        const sql = `
            ...
            FROM lines l
            LEFT JOIN railroads r ON r.id = l.railroad_id
            ${where}
            LIMIT 2000
        `;
        const { rows } = await pool.query(sql, params);
        ...
        res.json({
            ...
            features: rows.map(r => ({
                ...
                properties: {
                    ...
                    built_year: r.built_year,
                    abandoned_year: r.abandoned_year,
                    removed_year: r.removed_year,
                    railroad: r.railroad_name,
                    railroad_abbr: r.railroad_abbr,
                },
            })),
        });
    } catch (err) {
...
// =============================================================================
// GET /api/railroads — list of all railroads
// =============================================================================
app.get('/api/railroads', async (_req, res) => {
    try {
        const { rows } = await pool.query(
            `SELECT id, name, abbreviation, founded_year, dissolved_year, country
             FROM railroads ORDER BY name`
        );
        res.json(rows);
    } catch (err) {
        res.status(500).json({ error: (err as Error).message });
...
// =============================================================================
// GET /api/historic-maps — all maps ordered by year
// =============================================================================
app.get('/api/historic-maps', async (_req, res) => {
    try {
        const { rows } = await pool.query(
            `SELECT id, title, publisher, published_year, tile_url, thumbnail_url, source, license
             FROM historic_maps ORDER BY published_year`
        );
        res.json(rows);
    } catch (err) {
        res.status(500).json({ error: (err as Error).message });
...
// =============================================================================
// POST /api/historic-maps — add a new historic map
// =============================================================================
app.post('/api/historic-maps', async (req, res) => {
    try {
        ...
        const { rows } = await pool.query(
            `INSERT INTO historic_maps (title, publisher, published_year, tile_url, thumbnail_url, source, source_url, license,
             bbox)
...
// =============================================================================
// POST /api/lines — submit a new rail line
// =============================================================================
...
// =============================================================================
// POST /api/sites — submit a new site
// =============================================================================
app.post('/api/sites', async (req, res) => {
    try {
        ...
        const { rows } = await pool.query(
            `INSERT INTO sites (name, site_type, status, geom, built_year, closed_year, demolished_year, city, state_province, description)
             VALUES ($1, $2, $3, ST_SetSRID(ST_MakePoint($4, $5), 4326), $6, $7, $8, $9, $10, $11)
             RETURNING id, name, site_type, status`,
            [name, site_type, status, lng, lat, built_year || null, closed_year || null, demolished_year || null, city || null, state_province || null, description || null]
        );
        res.status(201).json(rows[0]);
    } catch (err) {
        res.status(500).json({ error: (err as Error).message });
